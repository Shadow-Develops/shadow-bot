...
module.exports = class extends Command {
	...
	async run(message, args) {
		...
		const addUser = await message.mentions.members.last() || await message.guild.members.cache.get(args[0]) || await message.mentions.roles.first() || await message.guild.members.fetch(args[0]) || await message.guild.roles.cache.find(rl => rl.name === args[0]) || await message.guild.roles.cache.find(rl => rl.id === args[0]) || await args[0].slice(0, 3);
		...
		if (message.member.roles.cache.has(supportRole) || message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
			...
			if (addUser) {
				...
				message.channel.send(`${addUser} has been added to the ticket.`);
			}
		} else {
